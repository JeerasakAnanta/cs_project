...
def update_user(
    ...
    db_user = crud.update_user(
        db, user_id=user_id, user=user, current_user=current_user
    )
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return db_user
...
def get_system_statistics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        active_users = (
            db.query(models.User).filter(models.User.is_active == True).count()
        )
        inactive_users = (
        ...
        total_guest_conversations = (
            db.query(models.GuestConversation)
            .filter(models.GuestConversation.is_deleted == False)
            .count()
        )
        ...
        user_messages = (
            db.query(models.Message).filter(models.Message.sender == "user").count()
        )
        bot_messages = (
            db.query(models.Message).filter(models.Message.sender == "bot").count()
        )

        user_guest_messages = (
            db.query(models.GuestMessage)
            .filter(models.GuestMessage.sender == "user")
            .count()
        )
        bot_guest_messages = (
            db.query(models.GuestMessage)
            .filter(models.GuestMessage.sender == "bot")
            .count()
        )
        ...
        like_feedbacks = (
            ...
            .filter(models.Feedback.feedback_type == "like")
            .count()
        )
        dislike_feedbacks = (
            db.query(models.Feedback)
            .filter(models.Feedback.feedback_type == "dislike")
            .count()
        )
        ...
        recent_conversations = (
            ...
            .filter(models.Conversation.created_at >= seven_days_ago)
            .count()
        )

        recent_guest_conversations = (
            db.query(models.GuestConversation)
            .filter(
                and_(
        ...
        recent_guest_messages = (
            db.query(models.GuestMessage)
            .filter(models.GuestMessage.timestamp >= seven_days_ago)
            .count()
        )

        # Machine separation statistics
        unique_machines = (
            db.query(models.GuestConversation.machine_id).distinct().count()
        )
        ...
        statistics = {
            ...
            "feedbacks": {
                ...
                "satisfaction_rate": round(
                    (like_feedbacks / total_feedbacks * 100)
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving system statistics: {str(e)}"
        )
...
def get_conversation_statistics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        recent_registered = (
            db.query(models.Conversation)
            .filter(models.Conversation.created_at >= thirty_days_ago)
            .count()
        )
