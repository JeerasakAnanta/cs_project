    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-green-600/20 to-green-800/20 border border-green-500/30 rounded-xl p-6">
          ...
          <p className="text-green-300 text-sm mb-3">การสนทนาทั้งหมด</p>
          <div className="space-y-1">
            <div className="flex justify-between text-xs">
              <span className="text-green-200">Registered</span>
              <span className="text-white font-medium">{formatNumber(systemStats.conversations.total_registered)}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-green-200">Guest</span>
              <span className="text-white font-medium">{formatNumber(systemStats.conversations.total_guest)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-600/20 to-purple-800/20 border border-purple-500/30 rounded-xl p-6">
          ...
          <p className="text-purple-300 text-sm mb-3">ข้อความทั้งหมด</p>
          <div className="space-y-1">
            <div className="flex justify-between text-xs">
              <span className="text-purple-200">User</span>
              <span className="text-white font-medium">
                {formatNumber(systemStats.messages.user_messages_registered + systemStats.messages.user_messages_guest)}
              </span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-purple-200">Bot</span>
              <span className="text-white font-medium">
                {formatNumber(systemStats.messages.bot_messages_registered + systemStats.messages.bot_messages_guest)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-600/20 to-amber-800/20 border border-amber-500/30 rounded-xl p-6">
          ...
          <p className="text-amber-300 text-sm mb-3">ความพึงพอใจ</p>
          <div className="space-y-1">
            <div className="flex justify-between text-xs">
              <span className="text-amber-200">Likes</span>
              <span className="text-white font-medium">{formatNumber(systemStats.feedbacks.likes)}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-amber-200">Dislikes</span>
              <span className="text-white font-medium">{formatNumber(systemStats.feedbacks.dislikes)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-neutral-800/50 border border-neutral-700/30 rounded-xl p-6">
          ...
          <div className="space-y-3">
            {Object.entries(systemStats.users.role_distribution).map(([role, count]) => (
        ...
        <div className="bg-neutral-800/50 border border-neutral-700/30 rounded-xl p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-neutral-300">การสนทนาใหม่</span>
              <span className="text-green-400 font-medium">
                {formatNumber(systemStats.conversations.recent_total)}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-neutral-300">ข้อความใหม่</span>
              <span className="text-blue-400 font-medium">
                {formatNumber(systemStats.messages.recent_total)}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-neutral-300">เครื่องที่ใช้งาน</span>
            ...
            <div key={day.date} className="text-center">
              ...
              <div className="bg-neutral-700/50 rounded-lg p-2">
                ...
                <div className="text-xs text-neutral-400">
                  <div className="text-green-400">{formatNumber(day.registered)}</div>
      ...
      {userStats.top_users_by_conversations.length > 0 && (
        <div className="bg-neutral-800/50 border border-neutral-700/30 rounded-xl p-6">
          ...
          <div className="space-y-3">
            {userStats.top_users_by_conversations.slice(0, 5).map((user, index) => (
              <div key={user.username} className="flex items-center justify-between p-3 bg-neutral-700/30 rounded-lg">
                <div className="flex items-center space-x-3">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-neutral-400">การสนทนา</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-neutral-800/50 border border-neutral-700/30 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex items-center space-x-3 p-3 bg-green-500/20 border border-green-500/30 rounded-lg">
            ...
            <div>
              <div className="text-white font-medium">ระบบออนไลน์</div>
              <div className="text-green-300 text-sm">ทำงานปกติ</div>
            </div>
          </div>
          <div className="flex items-center space-x-3 p-3 bg-blue-500/20 border border-blue-500/30 rounded-lg">
            <Clock className="w-5 h-5 text-blue-400" />
            <div>
              <div className="text-white font-medium">อัปเดตล่าสุด</div>
          ...
          <div className="flex items-center space-x-3 p-3 bg-purple-500/20 border border-purple-500/30 rounded-lg">
            ...
            <div>
              <div className="text-white font-medium">เครื่องที่ใช้งาน</div>
