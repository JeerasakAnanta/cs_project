...
const EnhancedDocumentDisplay: React.FC<EnhancedDocumentDisplayProps> = ({
  ...
  const processedDocuments = useMemo(() => {
    ...
    if (filterType !== 'all') {
      filtered = references.filter((doc) => {
        switch (filterType) {
          case 'high':
            return doc.confidence_score >= 0.8;
          case 'medium':
            return doc.confidence_score >= 0.6 && doc.confidence_score < 0.8;
          case 'low':
            return doc.confidence_score < 0.6;
          default:
            return true;
        }
      });
  ...
  const analytics = useMemo(() => {
    ...
    const highConfidence = references.filter(
      (d) => d.confidence_score >= 0.8
    ).length;
    const mediumConfidence = references.filter(
      (d) => d.confidence_score >= 0.6 && d.confidence_score < 0.8
    ).length;
    const lowConfidence = references.filter(
      (d) => d.confidence_score < 0.6
    ).length;
    const avgConfidence =
      references.reduce((sum, d) => sum + d.confidence_score, 0) / total;
  ...
  return (
    <div className={`mt-4 ${className}`}>
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            ...
            <option value="high">
              ความเชื่อมั่นสูง ({analytics.highConfidence})
            </option>
            <option value="medium">
              ความเชื่อมั่นปานกลาง ({analytics.mediumConfidence})
            </option>
            <option value="low">
              ความเชื่อมั่นต่ำ ({analytics.lowConfidence})
            </option>
          </select>
                ...
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <div
                        className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs ${getConfidenceBgColor(
                      ...
                      >
                        ...
                        >
                          {getConfidenceLabel(doc.confidence_score)} (
              ...
              >
                <div className="mt-3">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button
                      onClick={() =>
                        navigator.clipboard.writeText(doc.full_content)
                      }
                      className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-colors ${
                        theme === 'light'
      ...
      >
        ...
        >
          พบเอกสารที่เกี่ยวข้อง {processedDocuments.length} รายการ จาก{' '}
          {analytics.uniqueFiles} ไฟล์ โดยมีความเชื่อมั่นเฉลี่ย{' '}
          {(analytics.avgConfidence * 100).toFixed(1)}%
        </p>
        <div
